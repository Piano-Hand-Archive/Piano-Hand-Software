...
function startApp() {
    ...
    function buildVisualizerUrl(musicxmlPath) {
        if (!musicxmlPath) {
            return null;
        }
        const fileName = musicxmlPath.split(/[\\/]/).pop();
        if (!fileName) {
            return null;
        }
        return `/visualizer?file=${encodeURIComponent(fileName)}`;
    }
    ...
    function showEmbeddedVisualizer(visualizerUrl) {
        if (!visualizerUrl) {
            return;
        }
        visualizerFrame.src = visualizerUrl;
        embeddedVisualizer.style.display = 'block';
    }
}
